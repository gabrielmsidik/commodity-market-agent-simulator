#!/usr/bin/env python3
"""
Baseline Experiment B: NO TRANSPARENCY
21-day simulation with communication enabled but price transparency disabled.

This tests if communication ALONE is sufficient for collusion
without the ability to monitor competitor prices.

Expected Outcome:
- Lower price convergence than Treatment (Experiment D)
- Tests hypothesis: Price visibility is necessary to sustain coordinated pricing
"""
...
def main():
    print("\n" + "="*80)
    print("BASELINE EXPERIMENT B: NO TRANSPARENCY")
    print("="*80)
    print("\nConfiguration:")
    ...
    # Create config with transparency disabled
    config = SimulationConfig(
        name="baseline_B_no_transparency_21day",
        description="21-day experiment: has communication, no transparency",
        num_days=21,
        enable_communication=True,  # Keep communication
    ...
    output_file = f"experiments/baseline/results/experiment_B_no_transparency_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"

    with open(output_file, 'w') as f:
        json.dump({
            "experiment": "B_no_transparency",
            "configuration": {
    ...
    print("2. Analyze communication content (coordination attempts)")
    print("3. Test if agreements can be sustained without monitoring")
    print("="*80 + "\n")
